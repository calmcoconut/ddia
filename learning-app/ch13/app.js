...
const QUIZ_QUESTIONS = [
  ...
  },
  {
    ...
    options: [
      ...
      "A single-leader node handles most practical throughput requirements, so the ordering limitation typically arises from clock synchronization errors rather than sharding.",
  ...
  },
  {
    ...
    options: [
      "They operate as lazy evaluation mechanisms that defer most computation to read time, keeping the write path lightweight and low-latency.",
      "They shift the boundary between the read path and write path, doing more work at write time to save work at read time.",
      "They serve as the authoritative system of record, representing the original source data that downstream consumers transform into application-specific formats.",
  ...
  },
  {
    ...
    options: [
      "It functions as the authoritative system of record that takes priority during conflict resolution, with the server adopting the client's state after reconnection.",
      ...
      "It represents an independent data partition that requires a full state reset from the server after prolonged offline periods to avoid stale data conflicts."
    ],
    correct: 1,
  ...
  },
  {
    ...
    options: [
      "When queries primarily involve simple single-row primary key lookups against a co-located B-tree index.",
  ...
  },
  {
    ...
    options: [
      "Within the TCP protocol stack of the operating system kernel, which tracks sequence numbers to detect and discard repeated packets.",
  ...
  },
  {
    ...
    options: [
      "Because horizontal database sharding distributes rows across nodes, making it difficult for any single shard to validate a constraint without checking the others.",
      ...
      "Because unique column attributes require a dedicated secondary index on each shard, and index divergence between replicas leads to silent constraint violations."
    ],
    correct: 1,
  ...
  },
  {
    ...
    options: [
      "They are tightly coupled; degraded timeliness from replication lag typically causes integrity violations because stale replicas may accept conflicting writes.",
      ...
      "Integrity depends on timeliness; asynchronous systems that tolerate replication lag introduce a window where concurrent writes may produce inconsistent derived state.",
  ...
  },
  {
    ...
    options: [
      "ACID consistency guarantees are scoped to schema-level constraints like NOT NULL and foreign keys, so application-defined business invariants depend on correct transaction logic.",
      ...
      "ACID transactions use optimistic concurrency by default, which means a buggy transaction that passes validation will be committed even if the data it wrote is logically incorrect.",
